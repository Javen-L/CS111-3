...
int main(int argc, char *argv[]) {
    ...
    if (!newContext)
        exit_runtime_failures("ERROR: Unable to get SSL context");
    
    SSL *sslClient = SSL_new(newContext);
    if (!sslClient)
        exit_runtime_failures("ERROR: Unable to complete SSL setup");
    ...
    int ssl = SSL_set_fd(sslClient, sockfd);
    if (!ssl)
        exit_runtime_failures("ERROR: Unable to set SSL file descriptor");

    if (SSL_connect(sslClient) != 1) {
        exit_runtime_failures("ERROR: SSL connection rejected");
    }
    
    char ID_buf[30];
    sprintf(ID_buf, "ID=%d\n", ID);
    if (SSL_write(sslClient, ID_buf, strlen(ID_buf)) <= 0)
    ...
    while (1) {
        ...
        if (socket.revents & POLLIN) {
            ...
            int rd = SSL_read(sslClient, command, 256);
            if (rd <= 0)
                exit_runtime_failures("ERROR: Unable to read from socket");
    ...
    if (SSL_write(sslClient, exitMsg, strlen(exitMsg)) < 0)
        exit_runtime_failures("ERROR: Unable to write to socket");
